...
class SDBOGE_RewardCartCleanup {
    ...
    function sdboge_remove_reward_with_trigger(string $cart_item_key, $cart): void {
        ...
        foreach ($cart->get_cart() as $key => $item) {
            ...
            $reward_trigger_product_id = (int) ($item['_sdboge_trigger_product_id'] ?? 0);
            $reward_trigger_variation_id = (int) ($item['_sdboge_trigger_variation_id'] ?? 0);

            if ($reward_trigger_product_id === $product_id && $reward_trigger_variation_id === $variation_id) {
